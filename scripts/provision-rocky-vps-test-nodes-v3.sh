...
recreate_database_from_source() {
  local source_db="$1"
  local target_db="$2"
  local dump_file="$3"

  if database_exists "$target_db"; then
    echo "Recreando base existente: $target_db"
  ...
  sudo -u postgres pg_restore --no-owner --no-acl --clean --if-exists -d "$target_db" "$dump_file"
}
...
for spec in "${NODE_SPECS[@]}"; do
  ...
  recreate_database_from_source "$source_db" "$database_name" "$dump_file"
...
for spec in "${NODE_SPECS[@]}"; do
  IFS='|' read -r node_id _node_label _database_name _api_port _route_path _source_db _dump_file <<<"$spec"
