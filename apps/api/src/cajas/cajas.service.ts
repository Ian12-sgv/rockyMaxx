...
export class CajasService {
      ...
      async (tx) => {
        ...
        return tx.diarioCaja.create({
          data: {
            ...
            FacturaFinal: normalized.ultimaFactura,
  ...
  private normalizeInput(payload: CreateCajaDto | UpdateCajaDto): NormalizedCajaInput {
    ...
    const status = this.resolveStatus(payload.status);
  ...
  private resolveStatus(value: number | undefined) {
    ...
    if (![0, 1, 2].includes(value)) {
      throw new BadRequestException("El status de caja debe ser 0, 1 o 2.");
    }
